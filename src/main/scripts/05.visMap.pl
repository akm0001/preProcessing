...
while (my $rec= <MAP>){
	...
	my ($sID,$sstart,$send,$pident,$queryCovSub,$sstrand)= split(/\|/,$sseqInfo);
	my $valIDMap="$qstart|$qend|$sID|$sstart|$send|$pident|$queryCovSub|$sstrand|$sseq";
...
foreach my $tRnaID (sort keys %qIDsMap) {
	...
	for (my $i=0;$i<=$#mappedList;$i++){
		my ($posQStart,$posQEnd,$subID,$posSStart,$posSEnd,$identity,$queryCovSubSeq,$strandInfo,$sSequence)=split(/\|/,$mappedList[$i]);
		...
		print "$subID\tRef:[$posQStart,$posQEnd] Read:[$posSStart,$posSEnd] I: $identity Q: $queryCovSubSeq S: $strandInfo\n";
		}
	print "\n";
	}
